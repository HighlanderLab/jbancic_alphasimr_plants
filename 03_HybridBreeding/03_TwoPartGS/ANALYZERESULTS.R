...
# Inbred Genetic Gain
plot(1:nCycles,rowMeans(matrix(df$meanG_inbred,ncol = max(df$rep))),type="l",
     main="Inbred genetic gain",xlab="Year",ylab="Yield")

# Hybrid Genetic Gain
plot(1:nCycles,rowMeans(matrix(df$meanG_hybrid,ncol = max(df$rep))),type="l",
     main="Hybrid genetic gain",xlab="Year",ylab="Yield")

# Inbred Variance
plot(1:nCycles,rowMeans(matrix(df$varG_inbred,ncol = max(df$rep))),type="l",
     main="Inbred genetic variance",xlab="Year",ylab="Variance")

# Hybrid Variance
plot(1:nCycles,rowMeans(matrix(df$varG_hybrid,ncol = max(df$rep))),type="l",
     main="Hybrid genetic variance",xlab="Year",ylab="Correlation")

# Selection accuracy
plot(1:nCycles,rowMeans(matrix(df$acc_sel,ncol = max(df$rep))),type="l",
     main="Selection accuracy in Product Development",xlab="Year",ylab="Correlation")

# Selection accuracy
plot(1:nCycles,rowMeans(matrix(df2$accPI,ncol = max(df$rep))),type="l",
...
# Correlation
plot(1:nCycles,rowMeans(matrix(df$cor,ncol = max(df$rep))),type="l",
