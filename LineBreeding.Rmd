...
An important note! We work backwards through the pipeline to avoid copying a population before performing operations on it. This is an important point because failing to do so is a common source of coding bugs with AlphaSimR. To explain further, consider what would happen if we went forwards through the pipeline instead. An incorrect first attempt at doing so might be to start with:
...
And we would have to go through a similar process for all the stages. We avoid this process by running backwards through the pipeline instead, as shown above in previous code chunks.

Finally, for each year we save results in the `output` data frame. We save mean genetic value and variance of genetic values from individuals in the DH stage, while for accuracy of selection we calculate it first at the HDRW stage where first phenotypes are available. Note that heritability of these phenotypes is very low, representing breeder's visual selection.
...
This completes simulation of a single year of the burn-in for a single replicate. Explore additional material available in R scripts.