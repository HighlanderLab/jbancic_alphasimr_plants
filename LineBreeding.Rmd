...
In this vignette we present AlphaSimR code for a line breeding program using DH technology, as would be used in wheat breeding.  Sections of this vignette mirror subsections 3.2 -- 3.5 in the Results section of the main text, and we suggest the reader refer to it while reading these sections of the main text. For simplicity, in this vignette we only display a single year of the burn-in for a single replicate. For code of the full multi-year, multi-replicate line breeding program, refer to the AlphaSimR code found in the supplementary material and on github (path: 01_LineBreeding/01_PhenotypicSelection/04_DoubledHaploid/). Also in the supplementary material and on github can be found simulation code for numerous other breeding programs (see Table A1 of main text for a list of breeding programs and their directory paths.)
...
We create founder parents and set a phenotype, reflecting evaluation in EYT stage. 
...
Our wheat breeding program consists of six breeding stages and hence, we need to generate six distinct breeding populations through this filling process. The first round runs through all stages, saving the population in the final (sixth) stage. The second round runs through the stages up to the fifth stage, saving the population at that stage. This process is repeated six times until all stages have been populated. 
...
### Update Parents
First we update our parents by replacing the 10 oldest parents with 10 new parents from EYT stage. Note that in the script files this step is taken with `source("UpdateParents.R")`.
...
NB - We work backwards through the pipeline to avoid copying a population before performing operations on it. This is an important point because failing to do so is a common source of coding bugs. To explain further, consider what would happen if we went forwards through the pipeline instead. An incorrect first attempt at doing so might be to start with:
